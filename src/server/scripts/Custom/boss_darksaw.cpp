  ...
  public:
	...
	struct boss_darksawAI : public ScriptedAI
	{
		...
		void KilledUnit(Unit* victim)
		{
			...
			{
				me->MonsterSay("Ja Ja , son inutiles, ",LANG_UNIVERSAL,NULL);
	...
	/*(*itr)->*/	++_current_orbs;	// Optionally, you also can do stuff with (*itr)-> here.
				...
				if (_current_orbs < ADD_MAX_ADDS) 
				{
					...
					//Summon Orb Add
					Creature* AstralOrb = DoSpawnCreature(500051, float(rand() % 47), float(rand() % 47), 0, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				{	
					...
					case EVENT_PLAYSOUND_CAST1A:
						...
						if (t==14803) { me->MonsterYell("No me toquen..",LANG_UNIVERSAL,NULL); }
						if (t==14771) { me->MonsterYell("Sin piedad..",LANG_UNIVERSAL,NULL); }
						if (t==14751) { me->MonsterYell("Tu arma..",LANG_UNIVERSAL,NULL); }
					...
					_chatter_timer = 0;
						...
						if (t==14795) { me->MonsterYell("Gente inutil de Destyne, simpre mal organizados por eso moriran hoy",LANG_UNIVERSAL,NULL); }
						if (t==14786) { me->MonsterYell("Crees que puedes matarme tu me serviras como exclavo.",LANG_UNIVERSAL,NULL); }
						if (t==14787) { me->MonsterYell("Sufran insectos!",LANG_UNIVERSAL,NULL); }
						...
						if (_chatter_timer > MIN_PAUSE_CHATTER)
						{
						  _chatter_timer = 0;
							...
							Talk(RAND(
								...
								if (t==14738) { me->MonsterYell("Has cruzado al mundo de los muertos en busca de respuestas. Deseas salvarte aliado, y has arriesgado vidas y miembros para estar aquí. Dejame ayudar.",LANG_UNIVERSAL,NULL); }
								if (t==14745) { me->MonsterYell("Pero ... Todavía no es tu hora de servirme. Sí, te espera un destino más grande. Poder ... Debes ser más poderoso antes de servirme..",LANG_UNIVERSAL,NULL); }
								if (t==14754) { me->MonsterYell("Sera tu ultimo dia..",LANG_UNIVERSAL,NULL); }
								if (t==14755) { me->MonsterYell("Sirveme y seras mas fuerte!",LANG_UNIVERSAL,NULL); }
								...
								if (t==14773) { me->MonsterYell("La misericordia es para los débiles.!",LANG_UNIVERSAL,NULL); }
								if (t==14792) { me->MonsterYell("Que la destrucción de este lugar sirva de lección. A todos los que se atreverían a oponerse al flagelo...",LANG_UNIVERSAL,NULL); }
							
								//if (t==14799) { me->MonsterYell("Come to me, crusaders. I will remake you!",LANG_UNIVERSAL,NULL); }
								if (t==14802) { me->MonsterYell("Deja tus brazos y entrega tus almas a DestynePvp..",LANG_UNIVERSAL,NULL); }
								if (t==14766) { me->MonsterYell("Ser advertido..",LANG_UNIVERSAL,NULL); }
					...
					case EVENT_FRENZY:
						...
						if (t==14800) { me->MonsterYell("Corre ante mis temibles creaciones..",LANG_UNIVERSAL,NULL); }
						if (t==14787) { me->MonsterYell("Sufre insecto!",LANG_UNIVERSAL,NULL); }
